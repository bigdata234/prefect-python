    ...
    try:
        if IC.has_key('ICRCardInfo') and \
            ...
            for i in IC['ICRCardInfo']:
                if i.has_key('CardState') and \
                        i['CardState'] not in ['', None, 'null', '/', 'Null' , u'注销' , u'未激活' , u'尚未激活']:
                    if i.has_key('CardLimitAmt') and i['CardLimitAmt'] not in null_type_list:
                        CardLimitAmt_list.append(float(i['CardLimitAmt']))
                    else:
                        pass
                    if i.has_key('CardUsedLimitAmt') and i['CardUsedLimitAmt'] not in null_type_list:
                        CardUsedLimitAmt_list.append(float(i['CardUsedLimitAmt']))
                    else:
                        pass
                else:
                    pass
